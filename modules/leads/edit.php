...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($data['first_name']) || empty($data['last_name'])) {
        ...
        // Update lead
        $updated = $leadObj->update($lead_id, $data);
        
        if ($updated !== false) {
            $success = 'Lead uppdaterad';
            // Refresh lead data
            $lead = $leadObj->findById($lead_id);
        } else {
            $error = 'Kunde inte uppdatera lead. Försök igen.';
        }
    }
}
...
<head>
    ...
    <title>Redigera lead - <?php echo APP_NAME; ?></title>
        ...
        <div class="row">
            ...
            <main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-4">
                <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                    <h1 class="h2">Redigera lead: <?php echo htmlspecialchars($lead['first_name'] . ' ' . $lead['last_name']); ?></h1>
                </div>
                ...
                <form method="POST">
                    ...
                    <button type="submit" class="btn btn-primary">Uppdatera lead</button>
                    <a href="view.php?id=<?php echo $lead['id']; ?>" class="btn btn-secondary">Avbryt</a>
                </form>
            </main>
        </div>
    </div>
